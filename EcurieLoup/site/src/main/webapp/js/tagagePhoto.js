(function($) {
	...
	$(tagButton).attr("class", "tagActivateButton");
	...
	$(span).append(cadreTag);

	$(span).bind("mouseover", {
	    cadre : cadreTag
	}, function(event) {
	    $(event.data.cadre).css({
		"border-style" : "solid"
	    });
	});
	$(span).bind("mouseout", {
	...
	if (options.admin) {
	    ...
	    $(deleteLink).attr("src", ctx + "/images/delete.png");
	    $(deleteLink).click(function() {
		$.ajax({
		    url : ctx + "/ws/albumPhoto/tags/" + tag.id,
